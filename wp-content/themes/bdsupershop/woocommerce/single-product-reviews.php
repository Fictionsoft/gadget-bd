<?php
/**
 * Display single product  (comments)
 *
...
?>
<div id="" class="woocommerce-">
	<div id="comments">
		<h2 class="woocommerce--title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				/* translators: 1:  count 2: product name */
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s  for %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
				_e( '', 'woocommerce' );
			}
		?></h2>
		...
		<?php else : ?>
			...
			<p class="woocommerce-no"><?php _e( 'There are no  yet.', 'woocommerce' ); ?></p>
